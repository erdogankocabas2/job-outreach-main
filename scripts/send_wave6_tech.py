#!/usr/bin/env python3
"""Send Wave 6 Tech Startups Campaign emails (35 leads) using Gmail SMTP App Password with 30s pacing."""
...
logger = logging.getLogger("send_wave6_tech")


def main():
    conn = db.get_connection()
    camp = conn.execute("SELECT * FROM campaigns WHERE name LIKE 'Wave 6%' ORDER BY id DESC LIMIT 1").fetchone()

    if not camp:
        logger.error("No Wave 6 campaign found in database")
        sys.exit(1)
    ...
    leads = conn.execute("""
        ...
        WHERE l.status = 'APPROVED' AND l.id >= 57
        ORDER BY l.id ASC
    """).fetchall()

    leads = [dict(l) for l in leads]
    logger.info("Found %d unsent Wave 6 tech leads to dispatch", len(leads))
    ...
    print("=" * 60)
    print(f"STARTING WAVE 6 TECH CAMPAIGN ({len(leads)} emails, 30s interval)")
    ...
    print("=" * 60)
    print(f"WAVE 6 TECH CAMPAIGN COMPLETED: {sent_count} sent, {fail_count} failed")
