#!/usr/bin/env python3
"""Send Wave 5 Gaming Campaign emails (21 leads) using Gmail SMTP App Password with 30s pacing."""
...
logger = logging.getLogger("send_wave5_gaming")


def main():
    conn = db.get_connection()
    camp = conn.execute("SELECT * FROM campaigns WHERE name LIKE 'Wave 5%' ORDER BY id DESC LIMIT 1").fetchone()

    if not camp:
        logger.error("No Wave 5 campaign found in database")
        sys.exit(1)
    ...
    leads = conn.execute("""
        ...
        WHERE l.status = 'APPROVED' AND l.id >= 36
    """).fetchall()

    leads = [dict(l) for l in leads]
    logger.info("Found %d unsent Wave 5 gaming leads to dispatch", len(leads))
    ...
    print("=" * 60)
    print(f"STARTING WAVE 5 GAMING CAMPAIGN ({len(leads)} emails, 30s interval)")
    ...
    for i, lead in enumerate(leads):
        ...
        try:
            ...
            logger.info(
                "[%d/%d] Sending gaming email to %s <%s> @ %s...",
    ...
    print("=" * 60)
    print(f"WAVE 5 GAMING CAMPAIGN COMPLETED: {sent_count} sent, {fail_count} failed")
